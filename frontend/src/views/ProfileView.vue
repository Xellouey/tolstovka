...
onMounted(() => {
  if (isInTelegram.value && window.Telegram?.WebApp) {
    ...
    if (tgUser) {
      userInfo.value = {
        ...
        phone: '',
        avatar: (tgUser as any).photo_url || null
      }
    }
  }
})
...
const shareProfile = () => {
  if (isInTelegram.value && window.Telegram?.WebApp) {
    ...
    const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(origin)}&text=${encodeURIComponent(shareText)}`
