...
7) Техническая архитектура сервера
- ВАЖНО: Используем только Express.js. Fastify НЕ используется.
- Любые реализации на Fastify должны быть немедленно удалены из кода.
- Порт разработки: 8081 (вместо 8080).
- Роуты только через Express Router, middleware только Express-совместимые.
- При обнаружении Fastify кода - сразу рефакторинг на Express.

8) Бот: сценарии и тексты-заглушки
...
10) Развёртывание (VPS) - Только Express.js
- VPS (Linux, например Ubuntu 22.04+), Node.js LTS, reverse proxy (Nginx) с TLS для https://tolsovka.vercel.app → Express backend (порт 8081).
- ВАЖНО: Никаких Fastify сервисов! Только Express.js + middleware.
- Статика (баннеры/товары) отдаётся напрямую через Nginx из каталога /uploads и/или /public.
- Express сервис как systemd/pm2, переменные окружения для секретов (токен бота и т.п.).
